...
{
    int rstep = (dr > sr) ? 1 : (dr < sr) ? -1
                                          : 0;
    int cstep = (dc > sc) ? 1 : (dc < sc) ? -1
                                          : 0;
...
bool isPawnMove(int sr, int sc, int dr, int dc)
{
    ...
    {
        if (board[dr][dc] != ' ' && !isSameColor(board[sr][sc], board[dr][dc]))
            return true;
    }
    return false;
}
...
bool isMoveValid(int sr, int sc, int dr, int dc)
{
    ...
    if (piece == 'B' || piece == 'b')
        return isBishopMove(sr, sc, dr, dc);
    if (piece == 'Q' || piece == 'q')
        return isQueenMove(sr, sc, dr, dc);
    if (piece == 'K' || piece == 'k')
        return isKingMove(sr, sc, dr, dc);
